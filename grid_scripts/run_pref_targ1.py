...
sbx=sandbox.Sandbox()
sbx.build_sandbox('/home/fsweijen/software/GRID_LRT/GRID_LRT/data/config/steps/pref_targ1.cfg')
sbx.upload_sandbox()
sbx.cleanup()
...
s = srmlist.srmlist()
with open('srm-target.txt', 'r') as f:
   for l in f.readlines():
           s.append(l.strip())

print '5) Slice the list into a dictionary grouped with a group size 1.'
g = s.sbn_dict(pref='SB', suff='_')
d = srmlist.slice_dicts(g, 1)

print '6) Create a token for each subband.'
ts = Token.TokenSet(th=th, tok_config='/home/fsweijen/software/GRID_LRT/GRID_LRT/data/config/steps/pr    ef_targ1.cfg')
...
ts.add_attach_to_list('/home/fsweijen/software/GRID_LRT/GRID_LRT/data/parsets/Pre-Facet-Target-1.parset', tok_list=None, name='Pre-Facet-Target-1.parset')
th.add_view('pref_targ1', cond='doc.PIPELINE_STEP=="pref_targ1"')

print '8) Create and launch the jobs.'
from GRID_LRT.Application import submit
j = submit.jdl_launcher(numjobs=len(d.keys()), token_type=th.t_type, wholenodes=False, parameter_step=4, NCPU=4)

with j:
   url = j.launch()
print 'pref_targ1 url:', url
